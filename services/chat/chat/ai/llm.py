...
class LLMRouter:
    ...
    def generate(self, messages: list, tier: str = "standard", temperature: float = 0.4, max_tokens: int = None) -> str:
        ...
        raise Exception("Todos los proveedores LLM configurados (Groq, Cerebras, OpenRouter) han fallado o no están configurados.")
...
def generate_ai_report(context_data: dict) -> str:
    ...
    try:
        return _run_completion(
            system_message="Eres NURA, una analista de negocio que habla de forma clara, cercana y sin tecnicismos.",
            prompt=prompt,
            temperature=0.3,
            tier="premium"
        )
...
def route_intent(question: str, context: dict, history: str) -> str:
    ...
    try:
        response_text = router.generate(
            messages=[
                {"role": "system", "content": "Eres el enrutador de intenciones de NURA. Responde ÚNICAMENTE con la palabra clave del agente seleccionado."},
                {"role": "user", "content": prompt},
            ],
            tier="fast",
            temperature=0.1,
